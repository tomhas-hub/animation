...
//add on 2020/3/30/
__device__ float4 lassoColor = { 1.0, 0.0, 1.0, 1.0 };	//opaque pink.
...
__global__ void	d_render(int winWidth, int winHeight, cudaExtent volumeSize, int maxVolumeDim, 
	...
	int buttonState, int windowTitleBarHeight, int ox, int oy,
	uint *d_output)
{
	//获得thread(x, y) id.
...
__device__ void backgroundGen(int winWidth, int winHeight, unsigned char *dev_lassoLabelImg, unsigned char *dev_bgImg, 
	int grayValue_bg, int increment,
	int2 initialSeedPos)
{
	dev_bgImg[initialSeedPos.x + initialSeedPos.y * winWidth] = grayValue_bg;
	int initialSeedIntensity = dev_lassoLabelImg[initialSeedPos.x + initialSeedPos.y * winWidth];
	...
	while (newseeds != oldseeds)
	{
		...
		{
			if (dev_bgImg[x + y * winWidth] > 0) //indicate dev_bgImg(x, y) is a seed.
			{
				//(1)find this seed's upper neighbor.
				int intensity = dev_lassoLabelImg[x + (y - 1) * winWidth];
				if ((intensity >= minValue_Seed) && (intensity <= maxValue_Seed))
				{
					newseeds = newseeds + 1;
					dev_bgImg[x + (y - 1) * winWidth] = grayValue_bg;
				}

				//(2)find this seed's lower neighbor.
				intensity = dev_lassoLabelImg[x + (y + 1) * winWidth];
				if ((intensity >= minValue_Seed) && (intensity <= maxValue_Seed))
				{
					newseeds = newseeds + 1;
					dev_bgImg[x + (y + 1) * winWidth] = grayValue_bg;
				...
				intensity = dev_lassoLabelImg[(x - 1) + y * winWidth];
				if ((intensity >= minValue_Seed) && (intensity <= maxValue_Seed))
				{
					newseeds = newseeds + 1;
					dev_bgImg[(x - 1) + y * winWidth] = grayValue_bg;
				}
	...
	uint *d_output)
{
	...
	else
	{
		...
		d_render << <gridSize, blockSize >> > (winWidth, winHeight, volumeSize, maxVolumeDim, totAniFrames, d_aniMatrix, theta, lightPos,
			contextOpen, buttonState, windowTitleBarHeight, ox, oy, d_output);
	}
}
